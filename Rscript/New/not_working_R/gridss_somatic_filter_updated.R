...
argp = add_argument(argp, "--scriptdir", default=ifelse(sys.nframe() == 0, "./", dirname(sys.frame(1)$ofile)), help="Path to libgridss_DURATION_updated.R script")
...
argv = parse_args(argp, argv=c(
  "--input", "/stornext/Home/data/allstaff/w/wong.b/R/DURATION_Gridss_result/18_TGEG_530_result/18_TGEG_530_merged_mark.sv.vcf",
  "--output", "/stornext/Home/data/allstaff/w/wong.b/R/DURATION_Gridss_result/18_TGEG_530_result/18_TGEG_530_merged_mark_out.sv.vcf",
  "--pairoutput", "/stornext/Home/data/allstaff/w/wong.b/R/DURATION_Gridss_result/18_TGEG_530_result/18_TGEG_530_merged_mark_out_paired.sv.bedpe",
  "--pairoutput_details", "/stornext/Home/data/allstaff/w/wong.b/R/DURATION_Gridss_result/18_TGEG_530_result/18_TGEG_530_merged_mark_out_paired.csv",
  "-f", "/stornext/Home/data/allstaff/w/wong.b/R/DURATION_Gridss_result/18_TGEG_530_result/18_TGEG_530_merged_mark_full.sv.vcf",
...
libgridssfile = paste0(argv$scriptdir, "/", "libgridss_DURATION_updated.R")
if (file.exists(libgridssfile)) {
  tmpwd = getwd()
  setwd(argv$scriptdir)
  source("libgridss_DURATION_updated.R")
  setwd(tmpwd)
} else {
  msg = paste("Could not find libgridss_DURATION_updated.R in", argv$scriptdir, " - please specify a --scriptdir path to a directory containing the required scripts")
  write(msg, stderr())
  print(argp)
  stop(msg)
}
...
# comment below are lines from new gridss_somatic_filter in github
...
# newly added from github script, may not work for this normal only script?
if (argv$plotdir != "") {
  fdf = data.frame(
    id=names(filters),
...
inv_link_df = linked_by_simple_inversion_classification(bpgr) %>%
  group_by(linked_by) %>%
 # line below / whole script in github use sourceId instead of vcfId
  mutate(pass=passes_final_filters(vcf[vcfId])) %>%
  mutate(pass=any(pass)) %>%
  ungroup() %>%
  filter(pass) %>%
...
event_link_df = bind_rows(
  ...
  mutate(
    max_supporting_fragment_count = max(ifelse(is.na(info(full_vcf[vcfId])$MATEID), info(full_vcf[vcfId])$BVF, info(full_vcf[vcfId])$VF)),
    min_supporting_fragment_count = min(ifelse(is.na(info(full_vcf[vcfId])$MATEID), info(full_vcf[vcfId])$BVF, info(full_vcf[vcfId])$VF)),
...
# Ploting circos plot forinterchromosomal SVs
library(circlize)
#change the format of the result from Granges into bedpe
out_bpgr_inter_bedpe = breakpointgr2bedpe(out_bpgr_inter)
plot_title=paste(gsub(pattern = ".gridss.vcf", "", basename(argv$input)),".jpeg",sep = "")
jpeg(file=plot_title)
circos.initializeWithIdeogram()
title(gsub(pattern = ".gridss.vcf", "", basename(argv$input)))
