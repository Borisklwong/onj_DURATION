...
setwd("/stornext/Home/data/allstaff/w/wong.b/R/R_script")
...
addVCFHeaders = function(vcf) {
  ...
  VariantAnnotation::fixed(header(vcf))$FILTER = unique(as(rbind(as.data.frame(VariantAnnotation::fixed(header(vcf)))$FILTER, data.frame(
    ...
    Description=c(
      ...
      "Imprecise variant",
      "Require read pairs support",
      "Require split reads support",
      "Require assemblies support breakpoint from remote breakend",
      "Require assemblies support",
      "Break end result",
...
gridss_overlaps_breakpoint_pon = function(gr,
    ...
    ...) {
  seqlevelsStyle(pongr) = "UCSC"
  seqlevelsStyle(gr) = "UCSC"
  hasHit = rep(FALSE, length(gr))
  if (!is.null(pongr)) {
    filteredpongr = pongr[pongr$score >= gridss.pon.min_samples]
    hits = findBreakpointOverlaps(gr, filteredpongr, sizemargin=NULL, restrictMarginToSizeMultiple=NULL, ...)
...
#####BW: In DURATION study, only tumour sample performed WGS, not normal thus remove "normalOrdinal" and "tumourOrdinal"
gridss_breakpoint_filter = function(gr, vcf, bsgenome, min_support_filters=TRUE, somatic_filters=TRUE, support_quality_filters=TRUE, pon_dir=NULL, pon_maxgap=4) {
	vcf = vcf[names(gr)]
	i = info(vcf)
	g = geno(vcf)
...
##  filtered = .addFilter(filtered, "af", gridss_bp_af(gr, vcf, tumourOrdinal) < gridss.min_af)
  ...
  filtered = .addFilter(filtered, "RP", i$RP==0)
  filtered = .addFilter(filtered, "SR", i$SR==0)
  filtered = .addFilter(filtered, "RAS", i$RAS==0)
  filtered = .addFilter(filtered, "AS", i$AS==0)
...
annotate_overlaps = function(query, subject, ..., group_by_col_name="sampleId") {
	hits = findBreakpointOverlaps(query, subject, ...) %>% as.data.frame()
